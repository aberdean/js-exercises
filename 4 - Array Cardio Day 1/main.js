    ...
    const sixteenCentury = inventors.filter(inventor => inventor.year >= 1500 && 
                                                        inventor.year < 1600);
    console.table(sixteenCentury);
    ...
    const firstToLastBorn = inventors.sort((inventor1, inventor2) => {
      return inventor1.year - inventor2.year;
    });
    console.table(firstToLastBorn);

    // Array.prototype.reduce()
    // 4. How many years did all the inventors live?
    const totalYearsLived = inventors.map(inventor => 
                            inventor.passed - inventor.year)
                            .reduce((acc, currValue) => acc + currValue);
    console.log(totalYearsLived);

    // 5. Sort the inventors by years lived
    const longestToShortestLived = inventors.sort((inventor1, inventor2) => {
      return (inventor2.passed - inventor2.year) - (inventor1.passed - inventor1.year);
    });
    console.table(longestToShortestLived);

    // 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
    // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
    /** 
      * The following code needs to be run on the Wikipedia page listed above,
      * using the DevTools.
      * Uncomment before copy pasting it.
      */
    /*const category = document.querySelector('.mw-category');
    const links = [...category.querySelectorAll('a')];
    const boulevardsContainingDe = links.map(link => link.text)
    ...
    const vehicles = data.reduce((acc, currValue) => {
      if (!acc[currValue]) {
        acc[currValue] = 0;
      }
      acc[currValue] += 1;
